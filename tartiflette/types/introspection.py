...
class __TypeResolvers:
    ...
    @staticmethod
    async def description_resolver(_request_ctx, execution_data):
        return execution_data.parent_result.description

    @staticmethod
    async def fields_resolver(_request_ctx, execution_data):
        return execution_data.parent_result.fields

    @staticmethod
    async def interfaces_resolver(_request_ctx, execution_data):
        return execution_data.parent_result.interfaces

    @staticmethod
    async def possible_types_resolver(_request_ctx, execution_data):
        return execution_data.schema.possible_types[execution_data.parent_result.name]
...
__Type = GraphQLObjectType(
    ...
    fields=OrderedDict([
        ...
        ('name', GraphQLField(
            name="name",
            gql_type="String",
            resolver=__TypeResolvers.name_resolver
        )),
        ('description', GraphQLField(
            name="description",
            gql_type="String",
            resolver=__TypeResolvers.description_resolver
        )),
        ('fields', GraphQLField(
            name="fields",
...
__Schema = GraphQLObjectType(
    ...
    fields=OrderedDict([
        ('types', GraphQLField(
            ...
            gql_type=GraphQLNonNull(GraphQLList(GraphQLNonNull("__Type"))),
...
async def type_resolver(_request_ctx, execution_data: ExecutionData):
    ...
    gql_type = execution_data.schema._gql_types[typename]
    return gql_type
